...
function is31Flavors(arrayIndex){
   if(arrayIndex.length === 31){
        return true;
   }else {//if(arrayIndexCount != 31){
       return false;
   }
}
...
function removeLastFlavor(Popped){
   Popped.pop("Vanilla Burnt Almond");
   return Popped;
}
...
function removeFlavorByName(arry, stringo){
    for(let i in arry){
        if(arry[i] === stringo){
            arry.splice(i,1);
        }
